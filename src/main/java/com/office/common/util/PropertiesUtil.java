...
public class PropertiesUtil {
	...
	private static final String PROPERTIES_EGIS_FILE_NAME = "context-core.properties";
	...
	static {
		...
		try {
			...
			throw new RuntimeException("cann't load property file from classpath:" + PROPERTIES_EGIS_FILE_NAME + "!",
					e);
		} finally {
			try {
				if (scmsStream != null) {
	...
	public static String getProperties(String propertiesName, String key) {
		...
		try {
			...
			throw new RuntimeException("cann't load property file from classpath:" + propertiesName + "!", e);
		} finally {
			try {
				if (inputStream != null) {
					inputStream.close();
